...
public class XmlFilter {
    ...
    public boolean check(String string) {
        ...
        if(doc != null && doc.getNodeName().equals("filter")) {
            return evaluateSingleNode(doc.getChildNodes().item(1));
        }
    ...
    private boolean evaluateOr(Node node) {
        for(int i = 0; i < node.getChildNodes().getLength(); i++) {
            ...
            if(isCorrectNode(child) && evaluateSingleNode(child)) {
                return true;
            }
        }
        return false;
    ...
    private boolean evaluateNot(Node node) {
        for(int i = 0; i < node.getChildNodes().getLength(); i++) {
            ...
            if(isCorrectNode(child)) {
                boolean df = !evaluateSingleNode(child);
                return df;
            }
        }
        return true;
