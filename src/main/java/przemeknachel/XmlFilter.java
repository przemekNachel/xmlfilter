...
public class XmlFilter {
    ...
    private boolean evaluateCheck(Node node) {
        ...
        System.out.println("type: " + type + " field: " + field + " value: " + value);
        switch (type) {
            case "equals":
                return getValue(field).equals(value);
            case "in":
                return Arrays.asList(value.split(",")).contains(getValue(field));
            case "matches":
                return getValue(field).matches(value);
        }
        return true;
    }
